...
class MosdacIngestionPipeline:
    ...
    def __init__(self, api_token: Optional[str] = None):
        ...
        # Secure token authorization - omit header entirely if no token is provided
        self.headers = {
    ...
    def fetch_insat_raster_metadata(self, product_id: str = "3RIMG_L2B_LST") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error querying MOSDAC catalog for product {product_id}: {e}")
    ...
    def download_product_file(self, file_name: str, dest_folder: str = "data/raw") -> bool:
        ...
        if os.path.exists(dest_path):
            logger.info(f"File {file_name} already exists in raw cache. Skipping download.")
            return True
        ...
        try:
            logger.info(f"Downloading {file_name} from MOSDAC...")
            ...
            logger.error(f"Failed to download file {file_name} from MOSDAC: {e}")
            if os.path.exists(dest_path):
